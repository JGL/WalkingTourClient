...
function draw() {
  ...
  for (let index = 0; index < theImagesDisplay.length; index++) {
    if (theImagesDisplay[index]) {
      image(theImages[index], index * 40, 40);
    }
  }
}
...
function positionChanged(position) {
  ...
  for (let index = 0; index < tour.waypoints.length; index++) {
    ...
    let distanceFromWaypointToHere = calcGeoDistance(position.latitude, position.longitude, currentWaypointLatitude, -currentWaypointLongitude, 'km');
    distanceFromWaypointToHere *= 1000; //multiply distance by 1000 to get distance in metres

    console.log(`The ${index} range from here ${distanceFromWaypointToHere}, the range of the waypoint is ${currentWaypointRange}.`);

    theImagesDisplay[index] = (distanceFromWaypointToHere < currentWaypointRange);
  }
}
